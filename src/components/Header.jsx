...
import footerLogo from '../logofinal.png';
// Add Quicksand font
import '../index.css';
    ...
    <header className="bg-gradient-to-r from-[#fff8ec] via-[#f5f9ef] to-[#fff4e7] border-b border-primary-green/20 z-40 shadow-sm relative overflow-visible">
      <div>
        <nav className="grid grid-cols-3 items-center h-44 w-full p-0 m-0 overflow-visible">
          {/* Left Navigation */}
          <div className="hidden lg:flex flex-row items-center justify-start gap-6 pl-8 text-base min-w-0 relative z-20">
          ...
          <div className="flex flex-col items-center justify-center justify-self-center w-full gap-0 relative z-10 pointer-events-none">
            <Link to={ROUTES.home} className="focus:outline-none pointer-events-auto">
              <img src={footerLogo} alt="AGAF Logo" className="h-48 w-auto" />
            </Link>
          </div>

          {/* Right Navigation */}
          <div className="hidden lg:flex flex-row items-center justify-center gap-6 min-w-0 relative z-20 -ml-6">
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-primary-green/20 py-4 px-4 bg-gradient-to-r from-[#fff8ec] via-[#f5f9ef] to-[#fff4e7]">
            <div className="flex flex-col gap-4">
              <NavDropdown label="About" items={aboutDropdownItems} />
              <Link to={ROUTES.whatIsAdaptiveGolf} className="font-montserrat font-semibold text-sm text-primary-blue hover:text-[#4C9A63]">
                What is Adaptive Golf?
              </Link>
              <Link to={ROUTES.tryGolf} className="font-montserrat font-semibold text-sm text-primary-blue hover:text-[#4C9A63]">
                Programmes
              </Link>
              <Link to={ROUTES.competition} className="font-montserrat font-semibold text-sm text-primary-blue hover:text-[#4C9A63]">
                Competition
              </Link>
              <Link to={ROUTES.media} className="font-montserrat font-semibold text-sm text-primary-blue hover:text-[#4C9A63]">
                Media
              </Link>
              <Link to={ROUTES.connect} className="font-montserrat font-semibold text-sm text-primary-blue hover:text-[#4C9A63]">
                Contact
              </Link>
              <a
                href="https://www.instagram.com/adaptivegolfalliance/"
                target="_blank"
                rel="noopener noreferrer"
                className="text-primary-blue hover:text-[#4C9A63] flex items-center gap-2"
                title="Instagram"
              >
